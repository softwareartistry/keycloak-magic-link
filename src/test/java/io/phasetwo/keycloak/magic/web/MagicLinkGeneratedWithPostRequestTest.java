...
public class MagicLinkGeneratedWithPostRequestTest extends AbstractMagicLinkTest {
  ...
  @TestFactory
  public List<DynamicContainer> testMagicLinkCreation()
    ...
    UserRepresentation user =
        Helpers.createUser(
            keycloak, testRealm.getRealm(), "user1", "user1@phasetwo.io", "Phase", "Two");
    ...
    request.setState("cypress-state-pre-generated");
     ...
     */
    var accessTokenCurlResponse =
        runHttpRequestWithCurl(
            "curl",
            "-X",
            "POST",
            "-H",
            ...
            "-d",
            "password=" + container.getAdminUsername(),
            "-d",
            "grant_type=password",
            "http://host.testcontainers.internal:"
                + container.getHttpPort()
    ...
    var accessTokenResponse =
        Helpers.mapper()
            .readValue(accessTokenCurlResponse, new TypeReference<Map<String, String>>() {});
    var magicLinkCurlResponse =
        runHttpRequestWithCurl(
            "curl",
            "-X",
            "POST",
            ...
            "-H",
            "Authorization: Bearer " + accessTokenResponse.get("access_token"),
            "-d",
            requestAsString,
            "http://host.testcontainers.internal:"
                + container.getHttpPort()
    ...
    MagicLinkResponse magicLinkResponse =
        Helpers.mapper().readValue(magicLinkCurlResponse, MagicLinkResponse.class);
  ...
  private static String runHttpRequestWithCurl(String... commandPart) {
    try (GenericContainer<?> curlContainer = new GenericContainer<>(CURL_IMAGE)) {
      curlContainer.withNetwork(network).withAccessToHost(true).withCommand(commandPart);
      curlContainer.start();
